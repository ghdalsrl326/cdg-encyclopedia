export const BUILDING = [
  ...
  },
  {
    ...
    description: `
    돈화문은 조선 중기에 세워진 창덕궁의 정문으로 창경궁의 정문인 홍화문과 함께 중요시된다. 돈화문은 현존하는 궁궐의 대문 중에서 가장 오래된 목조 건물로, 1412년(태종 12) 5월에 창건되었으며 임진왜란 때 소실되었다가 1609년(광해군 1)에 창덕궁이 복구되면서 문도 함께 세워졌다. 
    
    한편 1413년(태종 13)에는 이 문에 무게 1만 5천 근의 동종(銅鐘)을 걸었다는 조선왕조실록의 기사가 있다. 2층 문루에는 종과 북이 있어 정오(正午)와 인정(人定), 파루(罷漏)에 시각을 알려주었다고 한다. 정오를 알리기 위해 북을 치는데 이것을 오고(午鼓)라고 하며, 인정은 통행금지를 알리기 위해 종을 치는 것이고, 파루는 통행금지 해제를 알리기 위해 종을 치는 것을 말한다. 
    
    돈화문은 1963년 보물로 지정되었으며 정면 5칸, 측면 2칸의 중층의 다포식 우진각지붕의 건물이다.
    `,
    sns: [
  ...
  },
  {
    ...
    인정전 내부에는 정면에 임금님의 용상이 있고 그 뒤에는 나무로 만든 곡병과 곡병 뒤에는 일월오악도(日月五岳圖)라는 병풍이 있다.병풍에는 음양을 뜻하는 해와 달이 있으며 이는 다시 왕과 왕비를 상징한다. 그 아래 다섯 개의 산봉우리는 우리나라의 동·서·남·북·중앙의 다섯 산을 가리키며 이는 국토를 의미한다.이것은 임금이 중앙에서 사방을 다스리고, 음양의 이치에 따라 정치를 펼친다는 뜻을 담고 있기도 하다. 그리고 주변에는 유리창을 비롯하여 전구나 커튼 등 서양 장신구가 설치되어 있는데, 이는 대한제국기 외국과의 수교 후 다양한 외래 문물이 들어 온 것이며, 1907년 순종이 덕수궁에서 창덕궁으로 이어한 후에 인정전의 실내바닥이 전돌에서 마루로 바뀌고, 전구가 설치되는 등 부분적인 변화가 일어났다.
    `,
    sns: [
  ...
  },
  {
    ...
    description: `
    선정전은 임금이 고위직 신하들과 함께 일상 업무를 보던 공식 집무실인 편전(便殿)으로, 지형에 맞추어 정전인 인정전 동쪽에 세워졌다. 선정전은 조선 초기에 창건되어 오랫동안 편전(便殿)으로 이용되어 아침의 조정회의, 업무보고, 국정세미나격인 경연 등 각종 회의가 이곳에서 매일 열렸으며, 후기로 가면서 역대 왕들의 빈전, 혼전으로 쓰이면서 희정당이 편전의 역할을 대신하였다. 처음에는 조계청(朝啓廳)이라 불렸으나, 1461년(세조 7)에 ‘정치는 베풀어야 한다’는 뜻의 선정전으로 이름을 바꾸었다.
    
    이 건물에서는 왕과 왕비가 크고 작은 행사를 벌였는데, 성종 때 왕비가 이곳에서 양로연(養老宴)을 베풀었고 친히 누에를 치는 일을 모범으로 보였으며, 왕이 주연을 열기도 하였다. 인조반정 때 창덕궁에 큰 불이 났을 적에 소실되었고, 1647년(인조 25) 에 중건되었다. 그 뒤 1674년(현종 15)에 수리한바 있고, 1803년(순조 3) 12월에 선정전 서행각(西行閣)이 소실된 것을 다음해 12월에 복건(復建)하였다.
  ...
  },
  {
    ...
    대조전은 현재 36칸으로 앞면 9칸·옆면 4칸이며, 지붕은 옆면에서 볼 때 여덟 팔(八)자 모양을 한 팔작지붕이다. 건물 가운데 3칸은 거실로 삼았으며, 거실의 동·서쪽으로 왕과 왕비의 침실을 두었다. 각 침실의 옆면과 뒷면에는 작은 방을 두어 사람들의 처소로 삼았다. 지붕에 용마루가 없으며 내부에 서양식 쪽마루와 유리창· 가구 등을 갖추어 놓아 현대적인 실내장식을 보이고 있다는 점이 특징적이다. 이 건물은 조선 왕실이 생활하던 건물로, 특히 대한제국기 황실의 내실(內室) 모습을 남겨두고 있는 점에서 중요하다.
    `,
    sns: [
  ...
  },
  {
    ...
    description: `
    성정각은 조선시대 왕세자가 거주했던 동궁(東宮)에 속한 전각으로 세자가 경서(經書)를 공부하던 창덕궁의 건물이다. ‘성정’이라는 명칭은 『대학(大學)』의 ‘성의(誠意)’, ‘정심(正心)’에서 앞 글자를 따서 붙였다. 건물은 희정당의 동남쪽에 자리하였으며, 왕세자가 하루에 3번 조강(朝講), 주강(晝講), 석강(夕講)의 서연(書筵, 冑筵)을 받았던 곳이다. 1705년(숙종 31)에 왕세자가 이곳에 있다가 뜰에 내려와 비답(批答)을 받았다는 기록이 있으므로, 이전에 건립되었던 것으로 보인다.
    
    성정각은 왕세자의 공간이었지만, 국왕이 사용하기도 하였다. 곧 1782년(정조 6)에 동궁전인 중희당(重熙堂)이 건립되면서, 이 건물은 초계문신의 시험을 치르거나 대신을 만나는 곳으로 활용되어 희정당(熙政堂)과 함께 편전으로 쓰이기도 하였다. 고종때 내의원(內醫院)을 전의사(典醫司)로 개편하였고, 순종 때 창덕궁을 개조하면서 내의원이 헐어 현판과 도구들을 이곳으로 옮겼다고 한다. 
    
    성정각은 앞면 6칸, 옆면 2칸으로, 익공계 팔작지붕의 형식이다. 서쪽부터 마루 2칸, 방 2칸을 두었고, 동쪽에는 남북 3칸의 누마루를 설치하였다. 성정각의 대문은 높은 솟을 대문이며 영현문(迎賢門)이라고 이름지었다. 정문으로 들어서면 건물의 오른쪽에 누마루가 있는데 남쪽에는 보춘정(報春亭)이라는 현판을 달았고 동쪽으로는 희우루(喜雨樓)라고 현판을 달았다. 우측 맞배지붕의 4칸짜리 전각에는 내의원이었음을 알리는 보호성궁(保護聖躬), 조화어약(調和御藥)이라는 현판이 걸려있다. 「동궐도(東闕圖)」에는 희우루 아래층에 벽을 설치한 모습이 보이는데, 지금은 흔적이 없는 상태이다.
    `,
    sns: [
  ...
  },
  {
    ...
    낙선재는 조선 24대 임금인 헌종이 후궁 경빈김씨와 할머니 순헌왕후를 위해 마련하여 조선 왕실의 권위를 확립하고 자신의 개혁의지를 실천하기 위해 지어진 장소이다. 헌종은 김재청의 딸을 경빈(慶嬪)으로 맞이하여 1847년(헌종13)에 낙선재를, 이듬해에 석복헌(錫福軒) 등을 지어 수강재(壽康齋)와 나란히 두었다. 낙선재는 헌종의 서재 겸 사랑채였고, 석복헌은 경빈의 처소였으며, 수강재는 당시 대왕대비인 순원왕후(23대 순조의 왕비)를 위한 집이었다. 후궁을 위해 궁궐 안에 건물을 새로 마련한 것은 매우 이례적인 일이다. 낙선재는 1884년 갑신정변 직후 고종의 집무소로 사용되었다. 이후 영친왕 이은의 부인 이방자 여사가 1966년부터 1989년까지 기거하였다. 
  ...
  },
  {
    ...
    title: "부용정(芙蓉亭)-芙蓉 : 연꽃",
    description: `
    창덕궁 부용정은 후원 초입에 위치한 주합루 남쪽 부용지(芙蓉池)에 면한 정자이다. 부용정은 2012년 보물 제1763호로 지정되었다. 부용정은 원래 숙종 33년(1707) 택수재(澤水齋)라는 이름으로 지어졌다가 정조 16년(1792)에 부용지를 고치면서 부용정(芙蓉亭)이라 부르게 되었다. 부용(芙蓉)은 연꽃을 뜻하며, 부용정은 휴식과 학문적 용도로 쓰인 건물이다. 이곳에서는 왕이 과거에 급제한 이들에게 주연을 베풀어 축하해 주기도 하였다. 정조 19년(1795)에는 수원 화성을 다녀온 정조가 이 정자에서 신하들과 낚시를 즐겼다고 한다. 또한 부용정 일대는 후원의 첫 번째 중심 정원으로, 휴식뿐 아니라 학문과 교육을 하던 장소였다.
  ...
  },
  {
    ...
    `,
    sns: [
      "https://www.instagram.com/p/CyNtNQLSwI-/?igshid=MzRlODBiNWFlZA==",
      "https://www.instagram.com/p/CySv62wSbQz/?igshid=MzRlODBiNWFlZA==",
      "https://www.instagram.com/p/CyVALsYyp1p/?igshid=MzRlODBiNWFlZA==",
    ],
  },
] as const;
